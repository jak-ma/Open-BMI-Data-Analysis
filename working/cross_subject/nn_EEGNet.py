...
def run_one_subject(test_id, X_all, Y_all, lr=1e-3, epochs=10, batch=32):
    n_ch = 20
    n_times = 626
    n_classes = 2
    X_train = np.concatenate([X_all[i] for i in range(len(X_all)) if i != test_id])
    Y_train = np.concatenate([Y_all[i] for i in range(len(Y_all)) if i != test_id])
...
def main():
    start = time.time()
    total = 54
    X_all, Y_all = load_all_subjects(session=1, total=total)

    subject_acc = []

    for test_id in range(2):
        print(f"\n========== Test Subject {test_id+1} / {total} ==========")
        acc = run_one_subject(test_id, X_all, Y_all, lr=1e-3, epochs=10, batch=16)
    ...
    plt.grid(True)
    plt.savefig("subject_acc_line.png")
    plt.show()
