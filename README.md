...
1. 首先是数据预处理中使用csp方法进行特征提取，经过实验探索最终选择了输出 4对特征（8个即n_components=8）时性能最佳（之前是2对）。
...
我们选择了官方的 `EEGNetv1` 模型用在我们的**跨被试**实验上，整体表现远远优于之前的几个传统机器学习模型
...
- 不跨session实验，感觉没有必要做实验，数据量更少，重点放在如何改进传统机器学习算法来优化这个实验的性能。
